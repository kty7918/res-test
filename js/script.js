let funcObj = {
  ...
  f_1:function(){
    ...
    //숫자카운팅-자바스크립트 내에서 제이쿼리 셋팅함
    //.count 클래스의 요소가 data-count 속성에 설정된 숫자까지3초동안 점점 숫자가 증가하는 카운트 애니메이션
    //대상.animate({속성:값, 속성:값, }, 시간, 이징, 끝나면 할일);

    $(function(){
      $('.count').each(function(){  
        //각각의 요소들을 반복적으로 순회(자바스크립트의 forEach와 동일)
        let crt = $(this); //각각의 요소
        let countTo = crt.attr('data-count');
        // console.log(countTo);
        $({
          countNum:crt.text() //초기숫자를 객체로 생성(현재요소의 텍스트내용)-> 이 객체의 countNum속성이 애니메이션 대상
        }).animate({ 
          countNum:countTo  //countNum 속성을 countTo 값까지 애니메이션
        },{
          duration:3000,
          easing:'linear',
          step:function(){ //애니메이션 중 매 스텝마다 실행
            crt.text(Math.floor(this.countNum))
          },
          complete:function(){  //애니매이션 끝난 후 실행
...
funcObj['f_0'](); //섹션0은 스크롤과 상관없이 첫뻔째는 먼저 실행이 되게함
...
window.addEventListener('scroll',function(){
  ...
  section.forEach((sec,i) => {
    if(scroll > section[i].offsetTop - outHeight &&
      scroll < section[i].offsetTop - outHeight + section[i].offsetHeight){funcObj['f_'+i]();
      // console.log(i);
      }
...
//스크롤이 섹션의 offsetTop에 정확히 맞을때만 감지하면 너무 늦음! 그래서 outHeight만큼 미리 앞당겨서 인식 -> 뷰포트에 다 들어오기 전에 이미 동작 현재스크롤위치가 "섹션이 뷰포트에 등장하기 시작할 시점" 과  '섹션이 뷰포트를 벗어나기 직전 시점' 사이에 있는지, 그때마다 섹션별 함수 f_i()를 실행
